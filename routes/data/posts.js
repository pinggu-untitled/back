...
// export async function getByIdWithMedia(conn, postId) {
//   conn.execute('UPDATE POST SET hits = hits + 1 where id = ?', [postId]);
//   return conn //
//     .execute(
//       'SELECT po.id, po.user, po.created_at, po.updated_at, po.title, po.content, po.longitude, po.latitude, po.hits, us.nickname, us.profile_image_url FROM POST as po join USER as us on po.user = us.id WHERE po.id = ? ORDER BY po.created_at desc',
...
export async function create(conn, post, mentions, hashtags, files) {
  ...
  for (const hashtag of hashtags) {
    ...
    const hashExist = await conn
      .execute(
        `SELECT COUNT(*) as count FROM HASHTAG WHERE content = '${hashtag.content}'`
      )
      .then((result) => result[0][0].count);

    if (hashExist === 0) {
      await conn.execute(`INSERT into HASHTAG (content) values (?)`, [
        hashtag.content,
      ]);
    }
  ...
  for (const mention of mentions) {
    await conn.execute(
      `INSERT into MENTION (post, sender, receiver) values (?, ?, ?)`,
      [Number(newPost.id), Number(USER_NUMBER), Number(mention.receiver)]
    );
  }

  if (files) {
    files.map(async (file) => {
      await fileRepository.create(conn, file, newPost.id);
    });
  }
...
export async function update(conn, post, mentions, hashtags, postId, files) {
  const updatePost = await conn
    .execute(
      `UPDATE POST set title = ?, content = ?, longitude = ?, latitude = ?, is_private = ? WHERE id = ?`,
      [
        post.title,
        post.content,
        post.longitude,
  ...
  await conn.execute(`DELETE FROM MENTION WHERE MENTION.post = ?`, [
    Number(postId),
  ]);
  ...
  for (const mention of mentions) {
    await conn.execute(
      `INSERT into MENTION (post, sender, receiver) values (?, ?, ?)`,
      [Number(postId), USER_NUMBER, Number(mention.receiver)]
    );
  }
